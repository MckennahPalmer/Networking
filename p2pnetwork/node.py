...
Trying for an peer-to-peer network application using Python

TODO: Let me know when a connection with a node has failed.
"""

class Node(threading.Thread):
    """Will implement a node that connects to other nodes and the original node is able
    to accept connections as well."""
    ...
    def __init__(self, host, port, id=None, callback=None, max_connections=1):
        """Create instance of a Node. If you want to implement the Node functionality with a callback, you should 
           provide a callback method. It is preferred to implement a new node by extending this Node class. 
            host: The host name or ip address that is used to bind the server to.
            port: The port number that is used to bind the server to.
            id: (optional)
            callback: (optional)
    ...
    def init_server(self):
        """Initialization of the server to receive connections. It binds to the given host and port."""
    ...
    def disconnect_with_node(self, node):
        """Disconnect the connection with the specified node. It stops the node and joins the thread.
...
>>>>>>> 0a402e274c5d80c6ecd74e99e3ad3470ddb94149
    ...
    def node_disconnected(self, node):
        """While the same node connection class is used, the class itself is not able to
    ...
    def node_reconnection_error(self, host, port, trials):
        """This method is invoked when a reconnection error occurred. The node connection is disconnected and the
           flag for reconnection is set to True for this node. This function can be over ridden to implement your
